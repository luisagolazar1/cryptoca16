...
/**
 * Generar datos históricos - CON OPCIÓN DE DATOS REALES
 */
export async function generateHistoricalData(crypto, days = 90, useRealData = false) {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    currentPrice = Math.max(currentPrice + randomChange, crypto.price * 0.5);
    prices.push(currentPrice);
    volumes.push(crypto.volume * (0.8 + Math.random() * 0.4));
  }
  
  return { prices: prices.reverse(), volumes: volumes.reverse(), timestamps: [] };
}
...
export function simulateInvestment(crypto, amount, days, historicalData) {
  try {
    ...
    console.error('Error in simulateInvestment:', error);
    return {
      initialAmount: amount,
      finalAmount: amount,
      profit: 0,
...
export function calculateInvestmentScore(crypto, historicalData) {
  try {
    ...
    console.error('Error in calculateInvestmentScore:', error);
    return 50;
  }
}
...
export async function rankInvestments(cryptos, useRealData = false) {
  try {
    ...
    console.error('Error in rankInvestments:', error);
    return cryptos;
  }
}
